...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, drop_prob):
        ...
        self.layers = nn.Sequential(
            ...
            nn.Dropout2d(drop_prob, inplace=False),
        ...
        if bilinear:
            self.layers = nn.Sequential(
                nn.Upsample(scale_factor=2, mode='bilinear',
                            align_corners=True),
                nn.Conv2d(self.in_chans, self.out_chans, kernel_size=1),
                nn.InstanceNorm2d(self.out_chans),
                nn.LeakyReLU(negative_slope=0.2, inplace=False),
            )
        else:
            self.layers = nn.Sequential(
                nn.ConvTranspose2d(self.in_chans, self.out_chans,
...
class UnetModel(nn.Module):
    ...
    def __init__(self, in_chans, out_chans, chans, num_pool_layers, drop_prob, bilinear):
        """
        Args:
        ...
        for i in range(num_pool_layers - 1):
            self.up_transpose_conv += [
                TransposeConvBlock(ch * 2, ch, bilinear)]
            self.up_conv += [ConvBlock(ch * 2, ch, drop_prob)]
            ch //= 2

        self.up_transpose_conv += [TransposeConvBlock(ch * 2, ch, bilinear)]
        self.up_conv += [
            nn.Sequential(
